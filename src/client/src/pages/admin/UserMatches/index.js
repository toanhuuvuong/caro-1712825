...
function UserMatches() {
  ...
  // --- Effect hook
  useEffect(() => {
    matchesHistoryAPI.getByUserId(userId)
    .then(data => {
      if(data.ok) {
  ...
  return(
    <>
      <Breadcrumbs currentItem="User matches" items={[{href: '/admin/list-users', name: 'List users'}]} />
      <div className="title">
            ...
            matchesHistory && matchesHistory.map((item, index) => {
              ...
              return (
                <tr key={item.id}>
                  ...
                  <td>{item.roomId}</td>
                  <td>{item.xPlayer === userId ? <Badge>You</Badge> : item.xPlayer}</td>
                  <td>{item.oPlayer === userId ? <Badge>You</Badge> : item.oPlayer}</td>
